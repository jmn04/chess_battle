...
class Bishop(ChessPiece):
    ...
    def draw_piece(self, offx=0, offy=0):
        if glb.mx < 800:
            x = Bishop.h_off + self.h * B_YOKO_OFF + offx
            y = Bishop.v_off + self.v * B_TATE_OFF + offy
        else:
            x = Bishop.h_off + self.h * B_YOKO_OFF - offx
            y = Bishop.v_off + self.v * B_TATE_OFF - offy
        glb.screen.blit(glb.img_piece, (x, y), area=rect_bishop[self.iswhite])

        if glb.mx < 800:
            x = Bishop.vs_hoff + (7 - self.h) * B_YOKO_OFF - offx
            y = Bishop.vs_voff + (7 - self.v) * B_TATE_OFF - offy
        else:
            x = Bishop.vs_hoff + (7 - self.h) * B_YOKO_OFF + offx
            y = Bishop.vs_voff + (7 - self.v) * B_TATE_OFF + offy
        glb.screen.blit(glb.img_piece, (x, y), area=rect_bishop[self.iswhite])
